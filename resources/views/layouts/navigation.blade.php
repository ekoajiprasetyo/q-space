<nav x-data="{ open: false, userPanelOpen: false }" class="fixed top-0 w-full z-50 transition-all duration-300 pointer-events-none md:pointer-events-auto">
        ...
        <div class="bg-white/80 backdrop-blur-md border border-white/50 shadow-lg shadow-slate-200/50 rounded-full px-6 h-16 flex items-center justify-between">
            ...
            <div class="flex items-center">
                ...
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        <x-dropdown-link :href="route('profile.edit')" class="hover:bg-teal-50 hover:text-teal-700 font-medium rounded-t-lg">
                            {{ __('Profile') }}
                        </x-dropdown-link>
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            ...
                            <x-dropdown-link :href="route('logout')"
                                    onclick="event.preventDefault();
                                                this.closest('form').submit();" class="text-red-600 hover:bg-red-50 hover:text-red-700 font-medium rounded-b-lg">
                                {{ __('Log Out') }}
                            </x-dropdown-link>
                        </form>
                    </x-slot>
    ...
    <div class="md:hidden fixed bottom-4 inset-x-0 px-4 z-[999] pointer-events-auto flex justify-center">
        ...
        >
            ...
            <div class="space-y-2">
                <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 w-full p-3 text-left bg-white hover:bg-gray-50 rounded-xl transition-colors border border-gray-100 text-gray-700 font-medium">
        ...
        <div class="w-full max-w-sm bg-slate-900/90 backdrop-blur-xl border border-white/10 shadow-2xl shadow-slate-900/50 rounded-2xl px-2 py-3 flex items-center justify-between relative z-20">
            ...
            <!-- User Panel Trigger -->
            <button @click="userPanelOpen = !userPanelOpen" class="flex-1 flex flex-col items-center gap-1 group py-1 focus:outline-none {{ request()->routeIs('profile.edit') ? 'text-teal-400' : 'text-slate-400' }}">
                <div class="w-8 h-8 rounded-full bg-gradient-to-r from-teal-400 to-cyan-500 flex items-center justify-center text-white text-[10px] font-bold shadow-md {{ request()->routeIs('profile.edit') ? 'ring-2 ring-teal-400 ring-offset-2 ring-offset-slate-900' : '' }}">
                    {{ substr(Auth::user()->name, 0, 1) }}
                </div>
                 <span class="text-[10px] font-bold {{ request()->routeIs('profile.edit') ? 'text-teal-400' : 'text-slate-500' }}">User</span>
            </button>
        </div>
    </div>
</nav>
